...
var SimpleNodeDb = function(options) {
    ...
    this.replicate = function(opts, callback) {
        log.info('create a replica with opts: ', opts);

        callback(new Error('replicate is not implemented yet'));
    };
...
SimpleNodeDb.createREPL = function(opts) {
    ...
    if (!opts.log) opts.log = require('simple-node-logger' ).createLogger();

    if (!opts.rowCallback) {
        opts.rowCallback = function(key, value) {
            opts.log.info( key, ':', value );
